...
def run_game():
    ...
    slow_print("")
    slow_print(f"{my_hero['name']} has traveled to fire mountain and encountered {villain_1['name']} (Level: {villain_1['level']} Health: {villain_1['health']})")
    slow_print("")
    battle(villain_1)
    if my_hero['health'] <= 0:
    ...
    level_up(villain_1)
    slow_print("")
    slow_print(f"{my_hero['name']} has traveled to the spirit temple and encountered {villain_2['name']} (Level: {villain_2['level']} Health: {villain_2['health']})")
    slow_print("")
    battle(villain_2)
    if my_hero['health'] <= 0:
        sys.exit("You must attempt your adventure again")
